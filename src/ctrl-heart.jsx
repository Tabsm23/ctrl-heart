...
export default function CtrlHeart() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, body: "I keep starting sentences I never finish because none of them get to the point. The point is I miss you and I'm angry at myself for that.", date: "Apr 18" },
    { id: 2, body: "You would have laughed at that thing that happened today. I hate that I still think of you when something funny happens.", date: "Apr 14" },
    { id: 3, body: "I'm not angry anymore. I'm just tired of the version of me that waited.", date: "Apr 9" },
  ]);
  ...
  const AuthScreen = ({ mode }) => (
    <div className="screen auth-screen">
      <button onClick={() => setScreen("landing")} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", display: "flex", alignItems: "center", gap: 6, marginBottom: 40, padding: 0 }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <button className="btn-social"><Icons.Google /><span>Continue with Google</span></button>
        <button className="btn-social"><Icons.Apple /><span>Continue with Apple</span></button>
        <div className="divider">or</div>
        {mode === "signup" && <input className="input" placeholder="Your name" />}
        <input className="input" placeholder="Email address" type="email" />
        <input className="input" placeholder="Password" type="password" />
        <button className="btn-primary" style={{ marginTop: 8 }}
          onClick={() => mode === "signup" ? setScreen("paywall") : (setScreen("app"), setAppTab("home"))}>
          {mode === "signup" ? "Create account" : "Sign in"}
        </button>
        {mode === "signup"
          ? <p style={{ textAlign: "center", fontSize: 13, color: "var(--ink-4)" }}>Already have an account? <button className="btn-ghost" onClick={() => setScreen("login")}>Sign in</button></p>
          : <p style={{ textAlign: "center", fontSize: 13, color: "var(--ink-4)" }}>New here? <button className="btn-ghost" onClick={() => setScreen("signup")}>Create account</button></p>
        }
      </div>
    </div>
  );
  ...
  const PaywallScreen = () => {
    ...
    return (
      <div className="screen paywall-screen">
        ...
        <button className="btn-primary" onClick={() => { setScreen("app"); setAppTab("home"); setTimeout(() => setScreen("onboard"), 100); }}>
          Get access — {selected.price}
        </button>
  ...
  const VaultScreen = () => {
          ...
          {modal?.type === "burn" && (
            <div className="modal-overlay" onClick={() => setModal(null)}>
              <div className="modal-sheet" onClick={e => e.stopPropagation()}>
                ...
                <button className="btn-danger" style={{ marginBottom: 10 }} onClick={() => {
                  setMessages(m => m.filter(msg => msg.id !== modal.id));
                  setModal(null); setViewMsg(null);
                }}>Yes, burn it</button>
    ...
    if (composing) {
      return (
        <div className="screen">
          <div style={{ padding: "56px 24px 0" }}>
            <button onClick={() => setComposing(false)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--ink-3)", display: "flex", alignItems: "center", gap: 6, marginBottom: 32, padding: 0 }}>
              <Icons.ArrowLeft /><span style={{ fontSize: 14 }}>Back</span>
            </button>
            ...
            <textarea className="input" rows={12} placeholder="Dear them..." value={composeText}
              onChange={e => setComposeText(e.target.value)} style={{ marginBottom: 16 }} />
            <button className="btn-primary" onClick={() => {
              if (composeText.trim()) {
                setMessages(m => [{ id: Date.now(), body: composeText.trim(), date: "Today" }, ...m]);
                setComposeText(""); setComposing(false);
              }
      ...
      <div className="screen">
        <div className="screen-header" style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
          ...
          <button className="btn-primary" style={{ width: "auto", padding: "10px 16px", fontSize: 13, borderRadius: 10, marginTop: 20 }}
            onClick={() => setComposing(true)}>+ Write</button>
        </div>
    ...
    <div className="app">
      ...
      {screen === "app" && (
        <>
          ...
          {!checkIn && appTab === "vault"     && <VaultScreen />}
